...
const renderTodoCard = ({ todo, completed }) => {
  ...
  return `
    <div 
      ...
      <h2
        class="mb-5 text-gray-900 dark:text-white before:inline-block before:w-4 before:h-4 before:border-gray-500 dark:before:border-white before:border-4 before:border-${borderColor} before:rounded-full before:relative before:top-0.5"
        >
        <span class="ps-1">${todo.split(' ').slice(0, 3).join(' ')}</span>
      </h2>
      <p class="text-gray-900 dark:text-white text-sm mb-4">${todo}</p>
